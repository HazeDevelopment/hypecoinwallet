...
const char DEFAULT_WALLET_FILE_NAME[] = "hypecoinwallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
	m_defaultPoolList << "hypecoin.pool.yt:1111";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("hypecoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hypecoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hypecoinwallet.desktop");
  ...
  res = autorunSettings.contains("HypeCoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("HypeCoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.hypecoin.hypecoin scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=HypeCoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("HypeCoinWallet", appPath);
    } else {
      autorunSettings.remove("HypeCoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\hypecoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:hypecoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\hypecoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\hypecoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("hypecoinwallet.cfg"));
