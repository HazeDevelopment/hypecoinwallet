...
private:
  ...
  const QDir m_hypecoinDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfHypeCoinBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _hypecoin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo hypecoinBlockchainInfo() const;
